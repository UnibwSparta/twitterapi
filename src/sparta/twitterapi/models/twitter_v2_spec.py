...
#   timestamp: 2024-02-22T17:14:04+00:00
"""Manual edits:
...
class TweetWithheld(BaseModel):
    ...
    country_codes: List[constr(pattern=r"^[A-Za-z]{2}$")] = Field(..., description="Provides a list of countries where this content is not available.", min_length=1)  # type: ignore
...
class UserSearchQuery(RootModel[constr(pattern=r"^[A-Za-z0-9_]{1,32}$")]):  # type: ignore
    root: constr(pattern=r"^[A-Za-z0-9_]{1,32}$") = Field(..., description="The the search string by which to query for users.")  # type: ignore


class UserTakedownComplianceSchema(BaseModel):
    event_at: AwareDatetime = Field(..., description="Event time.", examples=["2021-07-06T18:40:40.000Z"])
    user: User2
    withheld_in_countries: List[constr(pattern=r"^[A-Za-z]{2}$")] = Field(..., min_length=1)  # type: ignore
...
class Like(BaseModel):
    ...
    liking_user_id: Optional[constr(pattern=r"^[0-9]{1,19}$")] = Field(  # type: ignore
        None,
        description="Unique identifier of this User. This is returned as a string in order to avoid complications with languages and tools that cannot handle large integers.",
        examples=["2244994945"],
    )
...
class ListCreateResponse(BaseModel):
    data: Optional[Data2] = Field(None, description="A X List is a curated group of accounts.")
    errors: Optional[List[Problem]] = Field(None, min_length=1)
...
class Reply(BaseModel):
    ...
    exclude_reply_user_ids: Optional[List[constr(pattern=r"^[0-9]{1,19}$")]] = Field(None, description="A list of User Ids to be excluded from the reply Tweet.")  # type: ignore
...
class StreamingLikeResponse(BaseModel):
    data: Optional[Like] = None
    errors: Optional[List[Problem]] = Field(None, min_length=1)
    includes: Optional[Expansions] = None
