...
#   timestamp: 2023-07-13T13:53:07+00:00
"""Manual edits:

Change possible username length from 15 to 20:
pattern=r"^[A-Za-z0-9_]{1,15}$" to pattern=r"^[A-Za-z0-9_]{1,20}$".

Change profile_image_url because of blocked users with url length of 0:
class User: profile_image_url from Optional[AnyUrl] to Optional[str]
...
constr(regex=r"^[A-Z]{2}$") to constr(pattern=r"^[A-Za-z]{2}$") # type: ignore

Change minimum items in TweetTakedownComplianceSchema withheld_in_countries to 0
...
class CreateComplianceJobRequest(BaseModel):
    name: Optional[constr(max_length=64)] = Field(None, description="User-provided name for a compliance job.", example="my-job")  # type: ignore
...
class EntityIndicesInclusiveExclusive(BaseModel):
    end: conint(ge=0) = Field(..., description="Index (zero-based) at which position this entity ends.  The index is exclusive.", example=61)  # type: ignore
    start: conint(ge=0) = Field(..., description="Index (zero-based) at which position this entity starts.  The index is inclusive.", example=50)  # type: ignore


class EntityIndicesInclusiveInclusive(BaseModel):
    end: conint(ge=0) = Field(..., description="Index (zero-based) at which position this entity ends.  The index is inclusive.", example=61)  # type: ignore
    start: conint(ge=0) = Field(..., description="Index (zero-based) at which position this entity starts.  The index is inclusive.", example=50)  # type: ignore
...
class Annotation(EntityIndicesInclusiveInclusive):
    normalized_text: Optional[str] = Field(None, description="Text used to determine annotation.", example="Barack Obama")
    probability: Optional[confloat(ge=0.0, le=1.0)] = Field(None, description="Confidence factor for annotation type.")  # type: ignore
    type: Optional[str] = Field(None, description="Annotation type.", example="Person")
...
class HashtagFields(BaseModel):
    tag: str = Field(..., description="The text of the Hashtag.", example="MondayMotivation")
...
class ListPinnedRequest(BaseModel):
    list_id: constr(pattern=r"^[0-9]{1,19}$") = Field(..., description="The unique identifier of this List.", example="1146654567674912769")  # type: ignore
...
class RulesRequestSummaryItem(BaseModel):
    created: int = Field(..., description="Number of user-specified stream filtering rules that were created.", example=1)
    invalid: int = Field(..., description="Number of invalid user-specified stream filtering rules.", example=1)
    not_created: int = Field(..., description="Number of user-specified stream filtering rules that were not created.", example=1)
    valid: int = Field(..., description="Number of valid user-specified stream filtering rules.", example=1)


class RulesRequestSummaryItem1(BaseModel):
...
class RulesResponseMetadata(BaseModel):
    ...
    sent: str
    summary: Optional[Union[RulesRequestSummaryItem, RulesRequestSummaryItem1]] = None
...
class EditControls(BaseModel):
    editable_until: datetime = Field(..., description="Time when Tweet is no longer editable.", example="2021-01-06T18:40:40.000Z")
    edits_remaining: int = Field(..., description="Number of times this Tweet can be edited.")
    is_edit_eligible: bool = Field(..., description="Indicates if this Tweet is eligible to be edited.", example=False)
...
class TweetComplianceStreamResponseItem1(BaseModel):
...
class TweetLabelStreamResponseItem1(BaseModel):
    errors: List[Problem] = Field(..., min_length=1)
...
class TweetWithheld(BaseModel):
    ...
    country_codes: Set[constr(pattern=r"^[A-Za-z]{2}$")] = Field(  # type: ignore
...
class UrlImage(BaseModel):
    ...
    url: Optional[AnyUrl] = Field(None, description="A validly formatted URL.", example="https://developer.twitter.com/en/docs/twitter-api")
...
class PublicMetrics1(BaseModel):
    ...
    tweet_count: int = Field(..., description="The number of Tweets (including Retweets) posted by this User.")


class UserComplianceStreamResponseItem1(BaseModel):
    errors: List[Problem] = Field(..., min_length=1)
...
class UserIdMatchesAuthenticatedUser(RootModel):
    root: str = Field(..., description="Unique identifier of this User. The value must be the same as the authenticated user.", example="2244994945")
...
class UserProfileModificationObjectSchema(BaseModel):
    event_at: datetime = Field(..., description="Event time.", example="2021-07-06T18:40:40.000Z")
    new_value: str
    profile_field: str
    user: User2
...
class ComplianceJob(BaseModel):
    created_at: datetime = Field(..., description="Creation time of the compliance job.", example="2021-01-06T18:40:40.000Z")
    download_expires_at: datetime = Field(..., description="Expiration time of the download URL.", example="2021-01-06T18:40:40.000Z")
    download_url: AnyUrl = Field(..., description="URL from which the user will retrieve their compliance results.")
    id: constr(pattern=r"^[0-9]{1,19}$") = Field(..., description="Compliance Job ID.", example="1372966999991541762")  # type: ignore
    name: Optional[constr(max_length=64)] = Field(None, description="User-provided name for a compliance job.", example="my-job")  # type: ignore
    status: ComplianceJobStatus
    type: ComplianceJobType
    upload_expires_at: datetime = Field(..., description="Expiration time of the upload URL.", example="2021-01-06T18:40:40.000Z")
...
class Data1(BaseModel):
    ...
    dm_event_id: constr(pattern=r"^[0-9]{1,19}$") = Field(..., description="Unique identifier of a DM Event.", example="1146654567674912769")  # type: ignore
...
class DmEvent(BaseModel):
    ...
    event_type: str = Field(..., example="MessageCreate")
    id: constr(pattern=r"^[0-9]{1,19}$") = Field(..., description="Unique identifier of a DM Event.", example="1146654567674912769")  # type: ignore
    ...
    referenced_tweets: Optional[List[ReferencedTweet]] = Field(None, description="A list of Tweets this DM refers to.", min_length=1)
...
class DmMediaAttachment(BaseModel):
    media_id: constr(pattern=r"^[0-9]{1,19}$") = Field(..., description="The unique identifier of this Media.", example="1146654567674912769")  # type: ignore
...
class MatchingRule(BaseModel):
    id: constr(pattern=r"^[0-9]{1,19}$") = Field(..., description="Unique identifier of this rule.", example="120897978112909812")  # type: ignore
    tag: Optional[str] = Field(None, description="A tag meant for the labeling of user provided rules.", example="Non-retweeted coffee Tweets")
...
class ListModel(BaseModel):
    ...
    id: constr(pattern=r"^[0-9]{1,19}$") = Field(..., description="The unique identifier of this List.", example="1146654567674912769")  # type: ignore
...
class ListCreateResponse(BaseModel):
    data: Optional[Data2] = Field(None, description="A Twitter List is a curated group of accounts.")
    errors: Optional[List[Problem]] = Field(None, min_length=1)
...
class MentionFields(BaseModel):
    ...
    username: constr(pattern=r"^[A-Za-z0-9_]{1,20}$") = Field(..., description="The Twitter handle (screen name) of this user.")  # type: ignore
...
class Rule(BaseModel):
    id: Optional[constr(pattern=r"^[0-9]{1,19}$")] = Field(None, description="Unique identifier of this rule.", example="120897978112909812")  # type: ignore
    tag: Optional[str] = Field(None, description="A tag meant for the labeling of user provided rules.", example="Non-retweeted coffee Tweets")
    value: str = Field(..., description="The filterlang value of the rule.", example="coffee -is:retweet")
...
class Space(BaseModel):
    created_at: Optional[datetime] = Field(None, description="Creation time of the Space.", example="2021-07-06T18:40:40.000Z")
    creator_id: Optional[constr(pattern=r"^[0-9]{1,19}$")] = Field(  # type: ignore
        None,
        description="Unique identifier of this User. This is returned as a string in order to avoid complications with languages and tools that cannot handle large integers.",
        example="2244994945",
    )
    ended_at: Optional[datetime] = Field(None, description="End time of the Space.", example="2021-07-06T18:40:40.000Z")
    ...
    id: constr(pattern=r"^[a-zA-Z0-9]{1,13}$") = Field(..., description="The unique identifier of this Space.", example="1SLjjRYNejbKM")  # type: ignore
    ...
    is_ticketed: Optional[bool] = Field(None, description="Denotes if the Space is a ticketed Space.", example="false")
    lang: Optional[str] = Field(None, description="The language of the Space.", example="en")
    participant_count: Optional[int] = Field(None, description="The number of participants in a Space.", example=10)
    scheduled_start: Optional[datetime] = Field(
        None, description="A date time stamp for when a Space is scheduled to begin.", example="2021-07-06T18:40:40.000Z"
    )
    ...
    started_at: Optional[datetime] = Field(None, description="When the Space was started as a date string.", example="2021-7-14T04:35:55Z")
    state: State = Field(..., description="The current state of the Space.", example="live")
    subscriber_count: Optional[int] = Field(
        None, description="The number of people who have either purchased a ticket or set a reminder for this Space.", example=10
    )
    title: Optional[str] = Field(None, description="The title of the Space.", example="Spaces are Awesome")
    topics: Optional[List[Topic1]] = Field(None, description="The topics of a Space, as selected by its creator.")
    updated_at: Optional[datetime] = Field(None, description="When the Space was last updated.", example="2021-7-14T04:35:55Z")


class Topic(BaseModel):
    description: Optional[str] = Field(None, description="The description of the given topic.", example="All about technology")
    id: str = Field(..., description="Unique identifier of this Topic.")
    name: str = Field(..., description="The name of the given topic.", example="Technology")


class Geo1(BaseModel):
    coordinates: Optional[Point] = None
    place_id: Optional[str] = Field(None, description="The identifier for this place.", example="f7eb2fa2fea288b1")
...
class TweetCreateRequest(BaseModel):
    ...
    nullcast: Optional[bool] = Field(
        False, description="Nullcasted (promoted-only) Tweets do not appear in the public timeline and are not served to followers."
    )
...
class TweetNotice(BaseModel):
    application: str = Field(..., description="If the label is being applied or removed. Possible values are ‘apply’ or ‘remove’.", example="apply")
    details: Optional[str] = Field(None, description="Information shown on the Tweet label")
    event_at: datetime = Field(..., description="Event time.", example="2021-07-06T18:40:40.000Z")
    event_type: str = Field(..., description="The type of label on the Tweet", example="misleading")
...
class UrlFields(BaseModel):
    description: Optional[str] = Field(None, description="Description of the URL landing page.", example="This is a description of the website.")
    display_url: Optional[str] = Field(None, description="The URL as displayed in the Twitter client.", example="twittercommunity.com/t/introducing-…")
    expanded_url: Optional[AnyUrl] = Field(None, description="A validly formatted URL.", example="https://developer.twitter.com/en/docs/twitter-api")
    ...
    title: Optional[str] = Field(None, description="Title of the page the URL points to.", example="Introducing the v2 follow lookup endpoints")
    unwound_url: Optional[AnyUrl] = Field(
        None, description="Fully resolved url.", example="https://twittercommunity.com/t/introducing-the-v2-follow-lookup-endpoints/147118"
    )
    url: AnyUrl = Field(..., description="A validly formatted URL.", example="https://developer.twitter.com/en/docs/twitter-api")
...
class Geo(BaseModel):
    bbox: List[confloat(ge=-180.0, le=180.0)] = Field(..., example=[-105.193475, 39.60973, -105.053164, 39.761974], max_length=4, min_length=4)  # type: ignore
...
class Place(BaseModel):
    ...
    country: Optional[str] = Field(None, description="The full name of the county in which this place exists.", example="United States")
    country_code: Optional[constr(pattern=r"^[A-Za-z]{2}$")] = Field(None, description="A two-letter ISO 3166-1 alpha-2 country code.", example="US")  # type: ignore
    full_name: str = Field(..., description="The full name of this place.", example="Lakewood, CO")
    geo: Optional[Geo] = None
    id: str = Field(..., description="The identifier for this place.", example="f7eb2fa2fea288b1")
    name: Optional[str] = Field(None, description="The human readable name of this place.", example="Lakewood")
...
class Poll(BaseModel):
    ...
    id: constr(pattern=r"^[0-9]{1,19}$") = Field(..., description="Unique identifier of this poll.", example="1365059861688410112")  # type: ignore
    options: List[PollOption] = Field(..., max_length=4, min_length=2)
    voting_status: Optional[VotingStatus] = None


class TweetComplianceStreamResponseItem(BaseModel):
    data: Union[
...
class TweetComplianceStreamResponse(RootModel):
    root: Union[TweetComplianceStreamResponseItem, TweetComplianceStreamResponseItem1] = Field(..., description="Tweet compliance stream events.")


class TweetLabelStreamResponseItem(BaseModel):
    data: Union[TweetNoticeSchema, TweetUnviewableSchema] = Field(..., description="Tweet label data.")


class TweetLabelStreamResponse(RootModel):
    root: Union[TweetLabelStreamResponseItem, TweetLabelStreamResponseItem1] = Field(..., description="Tweet label stream events.")
...
class UserComplianceStreamResponseItem(BaseModel):
    data: Union[
...
class UserComplianceStreamResponse(RootModel):
    root: Union[UserComplianceStreamResponseItem, UserComplianceStreamResponseItem1] = Field(..., description="User compliance stream events.")


class AddOrDeleteRulesRequest(RootModel):
...
class Tweet(BaseModel):
    ...
    created_at: Optional[datetime] = Field(None, description="Creation time of the Tweet.", example="2021-01-06T18:40:40.000Z")
    edit_controls: Optional[EditControls] = None
    edit_history_tweet_ids: List[constr(pattern=r"^[0-9]{1,19}$")] = Field(..., description="A list of Tweet Ids in this Tweet chain.", min_length=1)  # type: ignore
    ...
    lang: Optional[str] = Field(None, description="Language of the Tweet, if detected by Twitter. Returned as a BCP47 language tag.", example="en")
    ...
    possibly_sensitive: Optional[bool] = Field(
        None, description="Indicates if this Tweet contains URLs marked as sensitive, for example content suitable for mature audiences.", example=False
    )
...
class User(BaseModel):
    ...
    protected: Optional[bool] = Field(
        None, description="Indicates if this User has chosen to protect their Tweets (in other words, if this User's Tweets are private)."
    )
    ...
    username: constr(pattern=r"^[A-Za-z0-9_]{1,20}$") = Field(..., description="The Twitter handle (screen name) of this user.")  # type: ignore
    verified: Optional[bool] = Field(None, description="Indicate if this User is a verified Twitter User.")
    verified_type: Optional[str] = Field(None, description="The Twitter Blue verified type of the user, eg: blue, government, business or none.")
